...
> **2026-05-03 v0 reduction applied to this plan.** See the design spec's "v0 Reduction" block. Net effect on this plan: 7 DDB tables → 4 (drop listing-versions, mcp-sessions, search-index); search-indexer Lambda → algolia-sync Lambda (still listings DDB-stream-driven); MCP Fargate task def + listener rule removed.

**Goal:** Provision the AWS infrastructure (4 DynamoDB tables, 1 S3 bucket, 1 Lambda Algolia-sync) and the Stripe Connect Express SDK + webhook scaffolding that the marketplace.isol8.co subsystems will depend on.
