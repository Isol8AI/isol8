...
**Date:** 2026-04-29 (v0 reduction applied 2026-05-03; Round 2 Isol8-internal scope + build applied 2026-05-04)
**Status:** APPROVED + REDUCED for v0 ship — see Round 1 + Round 2 reduction blocks below
...
## Round 2 reduction — Isol8-internal scope (2026-05-04)

After Round 1 reduced the v1 surface (MCP / listing-versions / search-index dropped, Algolia inserted), a Round 2 product call narrowed scope further: **the marketplace transacts only between Isol8 paying users**, even though the storefront at `marketplace.isol8.co` stays public for browse + SEO.

### What this changes

- **Sellers and buyers are 100% Clerk-signed-in Isol8 users.** Anyone clicking Buy / Publish / Deploy must auth through Clerk; the storefront serves as a public discovery layer + sign-up funnel for Isol8 itself.
- **Publishing happens inside the chat app**, not on the storefront. A new `<PublishAgentModal>` component on the AgentsPanel lets Pro users turn an agent they've already built into a marketplace listing in one focused modal flow. The standalone `/sell` storefront page is gone.
- **Buying is one-click container deploy.** Buyer clicks "Deploy" on a free listing or "Buy" on a paid one. Free path: deploy fires immediately and lands the agent in their Isol8 container's EFS via the existing `catalog_service.deploy_from_artifact()` plumbing. Paid path: Stripe Checkout → return to `/deploy-success?session_id=…` which auto-fires the deploy on mount. **No CLI installer.**
- **Distribution scope:** Container-only. Future "install on my laptop" path can come back as a Phase 2 CLI; not in v0.
- **`skillmd_adapter` removed.** Marketplace publishes only OpenClaw-format listings via `agent_export.export_agent_from_efs()` (snapshots the seller's `agents/{agent_id}/` dir from EFS).
- **Algolia removed.** Replaced with an in-process backend search service (`marketplace_search.py`): TTL-cached scan + tokenized scoring (slug exact +50, tag exact +30/token, name substring +20/token, description substring +5/token). Sufficient at Isol8-internal scale; swap to a hosted search service when listing volume exceeds the in-process pattern.
- **Takedowns reframe.** No public DMCA filing form exists at v0 (no claimant pool to file from). The takedown workflow is admin-initiated: a "Take down this listing" button on the admin listing-detail page opens a 3-input dialog (reason / basis_md / typed-confirm), backend writes a takedown row + cascades license revocation in one shot. The `/admin/marketplace/takedowns` route is now a read-only audit log of historical takedowns ordered by `decided_at` desc.
- **CLI auth endpoints + storefront `/cli/authorize` page deleted.** No CLI to authenticate.
- **Storefront `/sell` page deleted.** Publish flow moved to chat app.
- **Storefront `/skills` route + Skills tab deleted.** v0 only publishes openclaw-format; the skills tab was always empty.

### What survives

| Surface | Status |
|---|---|
| Public storefront browse (home, /agents, /listing/[slug]) | ✓ |
| Stripe Checkout for paid listings | ✓ |
| Stripe Connect Express sellers payouts (held balance + onboarding + Transfer flush) | ✓ |
| Admin moderation queue (review queue + approve/reject + listing detail with safety scan + admin-initiated takedowns + audit log) | ✓ |
| Backend services: marketplace_service, marketplace_search, marketplace_safety, agent_export, catalog_package, license_service, payout_service, takedown_service, webhook_dedup | ✓ |
| Backend routers: marketplace_listings (incl. /artifact-from-agent, /seller-eligibility, /listings/{slug}/deploy, /listings/search), marketplace_purchases (incl. /my-purchases), marketplace_payouts, marketplace_admin (incl. /listings/{id}/takedown, GET /takedowns audit log, /listings/{id}/preview with safety scan) | ✓ |

### Net infra at v0 ship

- **4 marketplace DDB tables**: listings, purchases, payout-accounts, takedowns. (Down from 7 in the original spec.)
- **1 S3 bucket**: `isol8-{env}-marketplace-artifacts`.
- **No standalone Lambdas** (the Algolia sync was removed in Round 2; no DDB-streams indexer needed for in-process search).
- **No standalone Fargate services** (MCP service removed in Round 1).

### Outstanding follow-ups carried forward

- **openclaw-slice synthesis at publish time** — currently `agent_export.export_agent_from_efs()` writes `openclaw_slice={}`. The deploy endpoint's empty-slice path extracts workspace files but does not register the new agent in the buyer's `openclaw.json` config. Buyer must register manually until this lands. Marked as `TODO(cut-11-followup)` in `catalog_service.py` and `marketplace_listings.py`.
- **Storefront test infrastructure** — `apps/marketplace/` has no Vitest/RTL setup. The publish modal + deploy-success page + listing-detail page have no client-side regression coverage; backend test coverage compensates for the contract.
- **Admin frontend test infrastructure** — same gap for `apps/frontend/src/app/admin/`.
- **Audit log richer columns** — the takedowns audit page renders raw `listing_id` UUIDs. Denormalize `slug` + `name` onto the takedown row at write time for at-a-glance triage.
- **`pointInTimeRecovery` deprecation** — all marketplace + existing 12 DDB tables use the deprecated form; stack-wide cleanup deferred.

---

## Round 1 reduction — v0 (2026-05-03)

The original design (preserved below for historical context) committed to
v1 scope of free + paid + dual-distribution + 7 DDB tables. After review
the cost-vs-value math for several pieces didn't hold at zero users; the
following cuts bring v0 to a tighter shape without changing the strategic
thesis:

| Cut | Reason | Status |
|-----|--------|--------|
| **MCP server (`apps/marketplace-mcp/`, MCP Fargate, /mcp/* listener, `marketplace-mcp-sessions` table, `delivery_method` field)** | Multi-tenant runtime is a costly v2 IP-protection on-ramp. CLI install delivers the install-simplicity wedge alone. ~$50-150/mo saved, security surface dropped. | Removed |
| **`marketplace-listing-versions` table** | The listings table is already keyed by (listing_id, version). Single-table is equivalent; older versions live as `status='retired'` rows. | Collapsed into `listings` |
| **DDB-based search infrastructure (`marketplace-search-index` table + `marketplace-search-indexer` Lambda + `marketplace_search.py` parallel-scan service)** | Hand-rolled in-house search with no typo tolerance, no facets, no instant-search UX. Algolia free tier (10k records / 100k ops/mo) covers v0–v1 comfortably with vastly better DX. | Replaced with Algolia (`marketplace-algolia-sync` Lambda + storefront `algoliasearch` lite client) |

**Net infra:** 7 marketplace tables → 4 (listings, purchases,
payout-accounts, takedowns). One Lambda (Algolia sync). No standalone
MCP service.

**Sections of this doc affected:** Architecture (lines ~149-221, MCP
boxes), Component boundaries (`apps/marketplace-mcp`,
`apps/marketplace-search-indexer`), Data Model (drop `marketplace-listing-versions`,
`marketplace-mcp-sessions`, `marketplace-search-index`), Distribution
Plan ("Hosted MCP server", lines ~419-456), Search infrastructure
(lines ~536-542 → swap to Algolia), Phase 2 migration (the v2-encrypted
foreshadowing now ships with whichever Algolia successor handles secure
delivery, not MCP).

**What did NOT change:** the manufactured-seller thesis, agents-first
positioning, Stripe Connect Express + separate-charges-and-transfers,
license-key lifecycle, refund/takedown workflows, US-only at v1. Below
is the original spec — read for context, but the cut list above
overrides where they conflict.
