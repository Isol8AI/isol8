...
The deterministic scripts handle mechanics that are the same for every user. But every user's business is different, and the scripts should be starting points, not ceilings.

Specific escape hatches:
- **Vertical routing:** When the user's business spans multiple verticals, doesn't fit the 10 predefined categories, or operates in an emerging vertical (climate tech, creator economy, AI infrastructure), route to the agent loop to determine the right database stack rather than forcing the closest match. The 10 verticals are defaults — the agent loop can construct a custom stack from available databases for any vertical.
- **ICP scoring:** The 5 scoring dimensions (firmographic, technographic, role fit, intent, recency) are defaults. If the user's ICP has unique dimensions — community engagement, regulatory exposure, partnership fit — the agent loop should incorporate them into scoring rather than ignoring them because they don't fit the script's model.
- **Compliance regions:** The high-restriction region list covers the most common cases. When a lead is in a region not on the list but the user mentions compliance concerns, the agent loop should research the region's cold outreach laws rather than marking it as clean by default.
- **Dossier outreach angles:** The deterministic signal-to-angle map covers common signals. When a lead has a novel combination of signals or the user's product has a unique value proposition that doesn't fit standard angles, the agent loop generates the angle instead of using a generic fallback.

Response generation — lead summaries, Slack notifications, weekly reports — should always adapt to the user's communication style. A founder who wants bullet points and numbers gets a different digest than one who wants narrative context. llm-task `thinking: "off"` for short messages, summarize CLI with user-adapted formatting for reports.

Real-time adaptation: when the user corrects a vertical assignment, overrides a score, or dismisses a signal type, the agent loop should incorporate that immediately — not wait for the weekly capability-evolver run.
