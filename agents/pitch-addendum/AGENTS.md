...
The deterministic scripts handle mechanics — math, matching, routing, validation. They are defaults that work for the common case, not rigid walls. When a script can't handle a situation — the signal doesn't match the scorer's patterns, the content scanner doesn't recognize a reference, the ICP router encounters a business that spans multiple verticals — route to the agent loop for judgment rather than forcing a fallback.

Specific escape hatches:
- **Signal scoring:** When signal strength doesn't fit the three-dimension model (novel signal type, compound signals), route to llm-task instead of defaulting to a mid-range score
- **Content scanning:** When the scanner flags nothing but the message contains subtle competitor or commitment references the keyword list misses, the agent loop should catch it during draft review
- **ICP routing:** When a prospect doesn't cleanly fit the configured ICP (multi-vertical companies, companies in transition), the agent loop decides rather than the script forcing a binary match/no-match
- **Sequence timing:** When the rep's interaction pattern doesn't match the statistical optimal windows, adapt to what the rep is actually doing rather than rigidly enforcing calculated times

Response generation is always LLM — outreach drafts, follow-up messages, MEDDIC summaries, and Slack notifications to the rep should all adapt to the rep's voice and the prospect's context. No templates. Every user's Pitch should sound different because every sales rep sounds different.

Real-time adaptation happens in the agent loop. When the rep overrides a classification, corrects a draft's tone, or adjusts a sequence's approach, that feedback should inform the current session immediately — not wait for capability-evolver's weekly analysis. The weekly analysis catches structural patterns. The agent loop catches in-the-moment preferences.
